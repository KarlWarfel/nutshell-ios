 ...
 */
 //   @IBOutlet weak var barButton3: UIBarButtonItem!
    ...
    @IBAction func barButton2Press(sender: AnyObject) {
        switch searchTextField.text! {
        case "💊":
            searchTextField.text = "💉"
        case "💉":
    ...
    @IBAction func barButton1Press(sender: AnyObject) {
        switch searchTextField.text! {
        case "🧀":
            searchTextField.text = "BGL"
        case "BGL":
    ...
    override func viewDidLoad() {
        ...
        if let sideMenu = self.sideMenuController()?.sideMenu {
            ...
            let revealWidth = min(ceil((280.0/320.0) * self.view.bounds.width), 299.0)
